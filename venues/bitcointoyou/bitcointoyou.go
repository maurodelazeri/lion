...
// Bitcointoyou internals
type Bitcointoyou struct {
	venue.Base
	*request.Handler
}

// Websocket is the overarching type across the Bitcointoyou package
...
func (r *Bitcointoyou) SetDefaults() {
	r.Enabled = true
	r.Base.LiveOrderBook = utils.NewConcurrentMap()
}

// Setup initialises the venue parameters with the current configuration
func (r *Bitcointoyou) Setup(venueName string, config config.VenueConfig, streaming bool, maxLevelsOrderBook int, mode ...pbAPI.SystemMode) {
