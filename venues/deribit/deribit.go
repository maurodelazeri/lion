...
// Websocket is the overarching type across the Deribit package
...
func (r *Deribit) SetDefaults() {
	r.Enabled = true
	r.Base.LiveOrderBook = utils.NewConcurrentMap()
}

// Setup initialises the venue parameters with the current configuration
func (r *Deribit) Setup(venueName string, config config.VenueConfig, streaming bool, maxLevelsOrderBook int, mode ...pbAPI.SystemMode) {
