...
// Websocket is the overarching type across the Kraken package
type Websocket struct {
	...
	*websocket.Conn
	...
	StreamingChannels   map[int64]string
}

// SetDefaults sets default values for the venue
func (r *Kraken) SetDefaults() {
	r.Enabled = true
	r.LiveOrderBook = utils.NewConcurrentMap()
}

// Setup initialises the venue parameters with the current configuration
func (r *Kraken) Setup(venueName string, config config.VenueConfig, streaming bool, maxLevelsOrderBook int, mode ...pbAPI.SystemMode) {
