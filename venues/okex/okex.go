...
// Websocket is the overarching type across the Okex package
...
func (r *Okex) SetDefaults() {
	r.Enabled = true
	r.Base.LiveOrderBook = utils.NewConcurrentMap()
}

// Setup initialises the venue parameters with the current configuration
func (r *Okex) Setup(venueName string, config config.VenueConfig, streaming bool, maxLevelsOrderBook int, mode ...pbAPI.SystemMode) {
